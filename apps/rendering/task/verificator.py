...
from apps.rendering.resources.imgcompare import compare_imgs, check_size
...
class RenderingVerificator(CoreVerificator):
    ...
    def make_advance_verification(self, img_file, subtask_info, subtask_id,
        ...
        print("START BOX: {}".format(start_box))
        print("CMP FILE: {}".format(cmp_file))
        print("CMP_START_BOX: {}".format(cmp_start_box))
        ...
        img.img.save(os.path.join(get_golem_path(), "verify", os.path.basename(img_file)))

        cmp_img = load_img(cmp_file)
        cmp_img.img.save(os.path.join(get_golem_path(), "verify",
        ...
        for pixel in pixels:
            if pixcolor == img.get_pixel(pixel):
                ...
                print "Image moved {}".format(movement)
                return True
        return False
